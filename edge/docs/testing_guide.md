...
프로젝트 `edge/` 디렉토리에 아래와 같이 `Dockerfile.edge`를 작성합니다.

> [!NOTE]
> **JetPack 아키텍처 지원 주의사항**
> - **JetPack 5.x 이하 (L4T R35.x)**: `nvcr.io/nvidia/l4t-pytorch:r35.2.1-pth2.0-py3`와 같이 레거시 l4t-pytorch 리포지토리를 이용합니다.
> - **JetPack 6.x 이상 (L4T R36.x)**: NVIDIA가 l4t-pytorch를 중단하고, 메인 `nvcr.io/nvidia/pytorch` 리포지토리에 통합하였습니다. 따라서 반드시 `-igpu` 접미사가 붙은 태그(예: `nvcr.io/nvidia/pytorch:24.03-py3-igpu`)를 지정해야 합니다.
...
# 2. 시스템 필수 의존성 및 이미지 라이브러리 설치 (pip는 베이스 이미지 내장 사용)
...
Mediamtx를 실행하기 전에 시스템에 Docker가 설치되어 있어야 합니다. OS별 Docker 설치 방법은 다음과 같습니다.

##### 1) Ubuntu 환경에서 Docker 설치
```bash
# Docker 공식 설치 스크립트 다운로드 및 실행
curl -fsSL https://get.docker.com -o get-docker.sh
sudo sh get-docker.sh

# Docker 서비스 활성화 및 자동 시작 설정
sudo systemctl enable --now docker

# 현재 사용자를 docker 그룹에 등록 (명령 수행 후 터미널 재실행 혹은 로그아웃 필요)
sudo usermod -aG docker $USER
newgrp docker
```

##### 2) Windows (PowerShell) 환경에서 Docker 설치
```powershell
# Windows 패키지 관리자(winget)를 사용해 Docker Desktop 설치
winget install Docker.DockerDesktop

# *설치 완료 후 시스템을 반드시 재부팅하고, WSL 2 백엔드 활성화 설정을 진행해 주십시오.
```
...
```bash
# [채널 1 송출] 16300000.avi 데이터를 복사하여 cam01 경로로 전송
ffmpeg -re -stream_loop -1 -i ./16300000.avi -c copy -f rtsp rtsp://localhost:8554/cam01

# [채널 2 송출] 16300002.avi 데이터를 복사하여 cam02 경로로 전송
ffmpeg -re -stream_loop -1 -i ./16300002.avi -c copy -f rtsp rtsp://localhost:8554/cam02
```

#### 옵션 B: 표준 H.264 및 AAC 코덱으로 실시간 트랜스코딩 전송 (호환성 극대화 ⭐️)
비디오 파일의 코덱이 구형(예: MPEG-4, Raw PCM 오디오 등)인 경우 수신 단(에지 파이프라인)에서 스트림 디코딩 에러가 발생할 수 있습니다. 
특히 라즈베리 파이 5(Raspberry Pi 5)와 같은 미니 PC/SBC 보드를 전송 노드로 사용할 경우, CPU 부하와 인코딩 지연(Latency)을 최소화하면서 RTSP 표준에 부합하는 H.264(비디오)와 AAC(오디오)로 재인코딩하여 안정적으로 송출해야 합니다.

* **표준 트랜스코딩 송출 (비디오+오디오):**
  ```bash
  ffmpeg -re -stream_loop -1 -i ./16300000.avi -c:v libx264 -preset ultrafast -c:a aac -f rtsp rtsp://localhost:8554/cam01
  ```

* **오디오 제외 트랜스코딩 송출 (비디오 전용 - 전송 장치 부하 최소화 ⚡️):**
  실시간 AI 추론에서 오디오 데이터가 필요하지 않은 경우, 오디오 스트림을 아예 전송하지 않도록 차단(`-an`)하여 전송 디바이스와 네트워크 대역폭 리소스를 절약할 수 있습니다.
  ```bash
  ffmpeg -re -stream_loop -1 -i ./16300000.avi -c:v libx264 -preset ultrafast -an -f rtsp rtsp://localhost:8554/cam01
  ```

* **주요 옵션 상세 설명**:
  * `-re`: 비디오 프레임 레이트에 맞추어 실시간 속도로 비디오를 스트리밍하는 필수 옵션입니다.
  * `-stream_loop -1`: 비디오 종료 시 무한 반복 스트리밍되도록 지정합니다.
  * `-c:v libx264 -preset ultrafast`: 원본 비디오를 표준 H.264 비디오 코덱으로 인코딩하되, 가장 빠른 속도(`ultrafast` 프리셋)를 주어 임베디드 단의 CPU 부담과 인코딩 지연 시간을 방지합니다.
  * `-c:a aac`: raw PCM 오디오나 기타 오디오 코덱을 RTSP 규격에 최적화된 AAC 코덱으로 변환하여 오디오 스트림 호환성을 제공합니다.
  * `-an`: 오디오 데이터(Audio Noise)를 완전히 제외하여 무소음 비디오만 송출하는 옵션입니다.
