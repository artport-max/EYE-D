...
- [ ] **Qdrant DB 적재 시 컬렉션 중복 조회 성능 최적화**
  - **현상**: 현재 `db_client.py`의 `validate_insert()`가 매 프레임 데이터를 저장할 때마다 Qdrant에 `GET /collections/...` API를 반복 호출하여 존재 여부를 확인하므로 실시간 처리 성능 저하 및 네트워크 병목을 유발함.
  - **개선안**: 기동 시점(`start()`)에 존재 여부를 최초 1회만 검증하거나, 클라이언트 내부에 검증 완료된 컬렉션 이름을 캐싱(`set`)하여 반복적인 HTTP GET 통신을 제거하도록 최적화 구현.
